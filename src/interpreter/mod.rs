...
impl<'a> InterpreterState<'a> {
    ...
    fn execute_line(&mut self, program: &Program<'a>) -> InterpreterResult<()> {
            ...
            Assign(target, expr) => {
                ...
                match target {
                    ...
                    AssignTarget::ByteAddr(addr) => {
                        ...
                        set_memory_u8(
                            store_address as usize,
                            &mut self.data,
                            value_to_assign as u8,
                        )?;
                    }
                }
            }
    ...
    fn evaluate_expr(&mut self, expr: &Expr<'a>, program: &Program<'a>) -> InterpreterResult<u32> {
        ...
        match expr {
            ...
            DerefByte(e) => {
                ...
                get_memory_u8(n, &self.data).map(u32::from)
            }
            VarAddress(name) => {
                Ok(get_var_address(name, &mut self.var_table, &mut self.data) as u32)
            }
        }
    }
    ...
    fn evaluate_funcall(
        ...
        intrinsics::try_execute_intrinsic(name, args, self).unwrap_or_else(|| {
            ...
            if let IntermediateLine::FunDeclaration(_, params) = &program.ir[self.instr_index] {
                ...
                // params.iter().enumerate().try_for_each(|(index, param)| {
                //     let addr = get_var_address(param, &mut self.var_table, &mut self.data);
                //     let arg_value = args.get(index).copied().ok_or_else(|| Error::ArgumentMismatch {
                //         expected: params.len(),
                //         got: args.len(),
                //         func_name: name.to_string(),
                //     })?;
...
fn get_memory_u32(index: usize, vec: &[u8]) -> InterpreterResult<u32> {
    if index + 3 >= vec.len() {
        Err(Error::U32ReadOutOfBounds {
            u32_read_index: index,
            memory_length: vec.len(),
        })
    } else {
        Ok(u32::from(vec[index]) << 24
...
fn set_memory_u32(index: usize, vec: &mut [u8], value: u32) {
...
fn get_memory_u8(index: usize, vec: &[u8]) -> InterpreterResult<u8> {
    vec.get(index).copied().ok_or(Error::U8ReadOutOfBounds {
        u8_read_index: index,
        memory_length: vec.len(),
    })
}

fn set_memory_u8(index: usize, vec: &mut [u8], value: u8) -> InterpreterResult<()> {
    if index < vec.len() {
        vec[index] = value;
        Ok(())
    } else {
        Err(Error::U8ReadOutOfBounds {
            u8_read_index: index,
            memory_length: vec.len(),
        })
    }
