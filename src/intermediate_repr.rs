...
fn convert_block<'a> (block: Block<'a>, counter: &mut u32) -> IntermediateBlock<'a> {
    block.into_iter().flat_map(|line| convert_line(line, counter)).collect()
}
    ...
    match line {
        ...
        Line::For(variable, start, end, body) => {
            ...
            block.extend(vec![
                ...
                IntermediateLine::JumpFalse(Expr::Lt(Box::new(Expr::Var(variable)), Box::new(end)), exit_label.clone())
            ]);
            block.extend(convert_block(body, counter));
            block.extend(vec![
                // var = var + 1
                IntermediateLine::Assign(variable, Expr::Add(Box::new(Expr::Var(variable)), Box::new(Expr::Literal(1)))),
